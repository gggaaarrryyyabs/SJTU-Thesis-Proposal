...
\begin{enumerate}[label=(\arabic{*})]
    ...
    \item 本课题的第二个研究内容是构造一类门限实现方案实现代价低的非线性组件。该研究旨在差分功耗分析下，利用门限实现方案解决敏感信息泄漏问题；同时非线性组件的门限实现方案的实现代价尽可能低。
    
    差分功耗分析能有效地攻击不具有侧信道防护能力的对称密码算法中的非线性组件。因此需要设计一类非线性组件，同时构造该组件对应的实现代价低的门限实现方案。 
    本课题拟设计一类非线性组件，并给出其具体的门限实现方案，该方案所需的最小份额数达到理论最低值。相比已知的门限实现所需的最小份额数达到理论最低值的非线性组件，设计的非线性组件具有更高的非线性度。

    根据已知的计算机搜索结果，部分非线性组件的门限实现方案所需的最小份额数无法达到理论最低值。
    对此，我们将对一类特殊的非线性组件进行分析，刻画其所需最小份额数达到理论最低值的门限实现方案的存在性与该组件密码学性质的关系，为构造非线性组件的门限实现方案提供理论支撑。
\end{enumerate}
\subsection{拟解决的关键问题}
\begin{enumerate}[label=(\arabic{*})]
    ...
    \item 对五类置换幂函数的Gowers范数进行研究，我们期望能完全确定几类置换幂函数的Gowers范数，用以衡量函数对低次逼近攻击的防护能力。该研究的难点之一是确定置换幂函数的$k$阶Gowers范数与该函数的$k-1$阶差分谱的联系；更进一步，对几类置换幂函数的$k-1$阶差分谱进行完全刻画是另一个技术难点。
    \item 对门限实现方案最小份额数为$t+1$的函数进行研究。该研究的难点之一是构造的非线性组件的非线性度不为零，即函数的分量函数没有仿射函数；更进一步，利用非线性度和差分均匀度与非线性组件门限实现方案最小份额数达到理论最低值的存在性建立联系更是有挑战性的问题。
\end{enumerate}
...
基于上述研究内容与关键问题，本课题的研究方法将主要是方案设计与理论分析证明。下面结合研究内容和关键问题详细说明本课题的研究方法，我们的研究方案会针对两部分研究内容进行规划。
...
\begin{enumerate}[label=\arabic{*})]
    ...
    \item 在设计一类非线性组件的门限实现方案过程中，利用“Direct Sharing”或者其他已知方法可以构造代数次数为$t$的置换函数的份额数为$t+2$的门限实现方案，
\end{enumerate}
...
\begin{enumerate}[label=(\arabic{*})]
    ...
    \item 构造一类S盒，其门限实现方案所需份额数达到理论最低$t+1$，且门限实现得到的新函数非线性度不为零；对一类特殊的函数是否具有份额数达到$t+1$的门限实现方案进行了刻画。
\end{enumerate}
